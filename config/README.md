# config — image-level config seed
...
| Path | Role |
|---|---|
| `config/` | **Single source of truth** for config. Edit here, then `./sync.sh` + restart. |
| `dsh-home/` (bind-mounted to `/root/.dsh`) | Runtime home dsh actually reads. Contains credentials, sessions, storages — **never commit**. |
| Image `/opt/dsh-seed` | Copy of `config/` at build time; reseeds a virgin home. |

Keep `config/` and `dsh-home/` in sync with `./sync.sh`. The script copies
